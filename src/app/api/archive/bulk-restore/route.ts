...
import { restoreArchivedComment, restoreArchivedPost } from '@backend/archive/service'
...
export async function POST(request: NextRequest) {
  ...
  const result = await runBulk(parsed.ids, (id) =>
    parsed.type === 'posts'
      ? restoreArchivedPost({ payload, archiveId: id, user })
      : restoreArchivedComment({ payload, archiveId: id, user }),
  )

  revalidatePath('/editor/queue')
  return NextResponse.json({
    success: result.failed.length === 0,
    message: `${result.succeeded.length} restored, ${result.failed.length} failed`,
    ...result,
  })
}
