      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="space-y-0">
          {features.map((feature, index) => (
            <div
              key={feature.number}
              className={`group flex flex-col gap-3 md:grid md:grid-cols-12 md:gap-8 py-6 sm:py-8 border-t border-border hover:bg-card/50 transition-colors duration-300 animate-fade-up stagger-${index + 1}`}
            >
              <div className="md:col-span-1 flex items-center gap-3 md:block">
                <span className="font-display text-xl sm:text-2xl md:text-3xl text-accent/60 group-hover:text-accent transition-colors">
                  {feature.number}
                </span>
                <h3 className="text-lg sm:text-xl md:hidden font-semibold tracking-tight group-hover:text-accent transition-colors">
                  {feature.title}
                </h3>
              </div>
